...
public class SQLConstants {
	...
	public static final String UPDATE_DELIVERY_STATUS = "update restaurant.take_outs set ordered_on=ordered_on, delivered_on=?, is_delivered=true where user_id=(select user_id from user where user_name=?) and is_delivered=false and order_type=?";
	...
	public static final String FIND_MAX_OFFER_ID = "select max(offer_id) from offers";
	public static final String GET_NEW_OFFER = "select food_1,food_2 from offers where offer_id=(select max(offer_id) from offers)";
	public static final String GET_FOODS_OF_CUSTOMER = "select take_outs.order_id, food_item.food_name from food_item,take_outs,orders where take_outs.is_delivered=false and (take_outs.order_type=? and take_outs.user_id=(select user_id from user where user_name=?)) and (food_item.food_id = orders.food_id and orders.order_id = take_outs.order_id)";
	public static final String GET_FOOD_QUANTITIES_OF_CUSTOMER = "select food_quantity from orders where order_id in(select order_id from take_outs where is_delivered=false and order_type=? and user_id=(select user_id from user where user_name=?))";
	...
	public static final String GET_FOOD_NAMES = "select food_name from food_item";
	public static final String GET_USER_NAMES = "select user_name from user";
	public static final String DELETE_ORDERED_FOODS = "delete from orders where order_id = ?";
	public static final String DELETE_ORDER = "delete from take_outs where order_id = ?";
	public static final String GET_ORDERS_OF_CUSTOMER = "select order_id from take_outs where take_outs.is_delivered=false AND take_outs.order_type = 'delivery' AND take_outs.user_id=(select user_id from user where user.user_name=?)";
	public static final String GET_CUSTOMERS_WITH_ORDERS = "select user_name from user where user_id in (select user_id from take_outs where is_delivered =false and order_type=?)";
	public static final String CHECK_REPEATING_FOOD_ITEMS = "select count(*) from orders where order_id = ? and food_id = (select food_id from food_item where food_name = ?)";
	public static final String UPDATE_REPEATING_FOOD_COUNT = "update orders set food_quantity = food_quantity + ? where order_id = ? and food_id = (select food_id from food_item where food_name = ?)";
}
